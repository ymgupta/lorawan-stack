...
import style from './gateway-general-settings.styl'
...
export default class GatewayGeneralSettings extends React.Component {
  ...
  async handleSubmit(values) {
    const { gtwId, updateGateway } = this.props

    return updateGateway(gtwId, values)
  }
  ...
  render() {
    ...
    return (
      <Container>
        <PageTitle className={style.visuallyHidden} title={sharedMessages.generalSettings} />
        <Row>
          <Col lg={8} md={12}>
            <Collapse
              title={m.basicTitle}
