...
function toggleTab(key){let tab=getTabByKey(key)
if(!tab)return
tabs.forEach((tab)=>{if(tab&&tab.classList.contains(activeClass)){tab.classList.remove(activeClass);}});tab.classList.add(activeClass);tabContent.forEach((item)=>{if(item&&item.classList.contains(activeClass)){item.classList.remove(activeClass);}
...
function getTabByKey(key){return document.querySelectorAll(`[data-tab="${key}"]`)[0];}
init();});