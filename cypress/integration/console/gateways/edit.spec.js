...
describe('Gateway general settings', () => {
  ...
  it('displays newly created gateway values', () => {
    ...
    cy.findByText('General settings', { selector: 'h1' }).should('be.hidden')
    ...
    cy.findDescriptionByLabelText('Gateway description').should('be.visible')
    cy.findByLabelText('Gateway Server address')
      .should('be.visible')
      .and('have.attr', 'value', gateway.gateway_server_address)
    cy.findDescriptionByLabelText('Gateway Server address').should('be.visible')
    cy.findByLabelText('Gateway status')
      .should('be.visible')
      .and('have.attr', 'value', 'false')
    cy.findDescriptionByLabelText('Gateway status').should('be.visible')
    ...
    cy.findByLabelText('Automatic updates')
      .should('be.visible')
      .and('have.attr', 'value', 'false')
    cy.findDescriptionByLabelText('Automatic updates').should('be.visible')
    cy.findByLabelText('Channel')
      .should('be.visible')
      .and('have.attr', 'placeholder')
      .and('eq', 'Stable')
    cy.findDescriptionByLabelText('Channel').should('be.visible')
